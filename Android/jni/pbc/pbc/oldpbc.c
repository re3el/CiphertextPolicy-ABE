...
tree_ptr tree_new2(int type, void *data) {
  ...
  switch(tok_type) {
    ...
    case t_id:
      ...
      if (tok_type == t_lparen) {
        if (parseexprlist(t = tree_new2(t_function, id))) {
          return t;
        }
        tree_delete(t);
        return NULL;
      } else {
        return tree_new2(t_id, id);
      }
    case t_string:
      lex();
      return tree_new2(t_string, id_new(word));
    case t_lparen:
      lex();
      t = parsesetexpr();
      if (!t) return NULL;
...
static tree_ptr parsepow(void) {
  ...
  if (tok_type == t_pow) {
    ...
    res = tree_new2(t_function, id_new("pow"));
...
static tree_ptr parsefactor(void) {
  ...
  if (tok_type == t_sub) {
    ...
    t = parsefactor();
    if (!t) return NULL;
    tree_ptr t1 = tree_new2(t_function, id_new("neg"));
  ...
  for (;;) {
    switch(tok_type) {
      case t_mul:
        ...
        t1 = tree_new2(t_function, id_new("mul"));
      ...
      case t_div:
        ...
        t1 = tree_new2(t_function, id_new("div"));
        darray_append(t1->child, res);
        darray_append(t1->child, t2);
        res = t1;
        break;
  ...
  for (;;) {
    switch(tok_type) {
      case t_add:
        ...
        //t1 = tree_new2(t_add, NULL);
        t1 = tree_new2(t_function, id_new("add"));
        darray_append(t1->child, res);
        darray_append(t1->child, t2);
        res = t1;
        break;
      case t_sub:
        ...
        //t1 = tree_new2(t_sub, NULL);
        t1 = tree_new2(t_function, id_new("sub"));
        darray_append(t1->child, res);
        darray_append(t1->child, t2);
        res = t1;
        break;
